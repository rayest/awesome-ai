...
export default function DetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const list = _getterMap["operations"]();
    ...
    return (
      <AdminShell>
        <div className="px-8 py-16 mx-auto max-w-[1280px] text-center">
          ...
          <Link href="/dictionary/operations">
...
function DetailPageInner({ op }: { op: any }) {
  ...
  const save = async () => {
    ...
    toast.success("已保存", { description: "工序档案 · " + op.id, duration: 3000 });
    setSaving(false);
  };
      ...
      <div className="px-8 py-8 mx-auto max-w-[1280px]">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-1.5 text-[14px] font-mono text-[var(--ink-mute)]">
            <Link href="/dictionary/operations" className="hover:text-[var(--ink)]">工序字典</Link>
            <span>›</span>
            <span className="text-[var(--ink)]">{String(v.name ?? op.name)}</span>
            <span className="text-[var(--ink-mute)] ml-2">· {op.id}</span>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="md" onClick={() => router.push("/dictionary/operations")}>返回列表</Button>
