...
export default function DetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const list = _getterMap["sample-types"]();
    ...
    return (
      <AdminShell>
        <div className="px-8 py-16 mx-auto max-w-[1280px] text-center">
          ...
          <Link href="/dictionary/sample-types">
...
function DetailPageInner({ op }: { op: any }) {
  ...
  const save = async () => {
    ...
    toast.success("已保存", { description: "样品种类档案 · " + op.id, duration: 3000 });
    setSaving(false);
  };
      ...
      <div className="px-8 py-8 mx-auto max-w-[1280px]">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-1.5 text-[14px] font-mono text-[var(--ink-mute)]">
            <Link href="/dictionary/sample-types" className="hover:text-[var(--ink)]">样品种类</Link>
            <span>›</span>
            <span className="text-[var(--ink)]">{String(v.name ?? op.name)}</span>
            <span className="text-[var(--ink-mute)] ml-2">· {op.id}</span>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="md" onClick={() => router.push("/dictionary/sample-types")}>返回列表</Button>
        ...
        <div className="border border-[var(--hairline)] rounded-md p-4 bg-[var(--card)]">
          ...
          <div className="grid grid-cols-2 gap-x-12">
            <Row label="样品种类 ID" value={op.id as any} source="auto_number" mono />
            <Row label="名称" value={
