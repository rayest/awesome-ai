...
export default function DetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const list = _getterMap["components"]();
    ...
    return (
      <AdminShell>
        <div className="px-8 py-16 mx-auto max-w-[1280px] text-center">
          ...
          <Link href="/dictionary/components">
...
function DetailPageInner({ op }: { op: any }) {
  ...
  const [v, setV] = useState<Record<string, string | number | boolean>>(() => ({
            location: op.location as any,
      name: op.name as any,
      isPart: op.isPart as any,
      remark: op.remark as any,
  }));
  ...
  const save = async () => {
    ...
    toast.success("已保存", { description: "部件档案 · " + op.id, duration: 3000 });
    setSaving(false);
  };
      ...
      <div className="px-8 py-8 mx-auto max-w-[1280px]">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-1.5 text-[14px] font-mono text-[var(--ink-mute)]">
            <Link href="/dictionary/components" className="hover:text-[var(--ink)]">部件配置</Link>
            <span>›</span>
            <span className="text-[var(--ink)]">{String(v.name ?? op.name)}</span>
            <span className="text-[var(--ink-mute)] ml-2">· {op.id}</span>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="md" onClick={() => router.push("/dictionary/components")}>返回列表</Button>
