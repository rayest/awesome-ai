...
function DetailPageInner({ op }: { op: any }) {
  ...
  const [v, setV] = useState<Record<string, string | number | boolean>>(() => ({
            size: op.size as any,
      heightCm: op.heightCm as any,
      euLetter: op.euLetter as any,
      ageOrNumeric: op.ageOrNumeric as any,
  }));
  ...
  const save = async () => {
    ...
    toast.success("已保存", { description: "尺码档案 · " + op.id, duration: 3000 });
    setSaving(false);
  };
      ...
      <div className="px-8 py-8 mx-auto max-w-[1280px]">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-1.5 text-[14px] font-mono text-[var(--ink-mute)]">
            <Link href="/dictionary/sizes" className="hover:text-[var(--ink)]">尺码配置</Link>
            <span>›</span>
            <span className="text-[var(--ink)]">{String(v.name ?? op.name)}</span>
            <span className="text-[var(--ink-mute)] ml-2">· {op.id}</span>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="md" onClick={() => router.push("/dictionary/sizes")}>返回列表</Button>
          ...
          <div className="grid grid-cols-2 gap-x-12">
            ...
            <Row label="身高 cm" value={
              <input type="number" value={Number(v.heightCm ?? op.heightCm)} onChange={(e) => upd("heightCm", Number(e.target.value))}
                className="bg-transparent border border-[var(--hairline)] rounded px-1.5 py-1 text-right tnum text-[var(--ink)] focus:outline-none focus:border-[var(--primary)] w-full font-mono" />
            } source="number" mono />
            <Row label="欧码" value={
              <input value={String(v.euLetter ?? op.euLetter)} onChange={(e) => upd("euLetter", e.target.value)}
                className="bg-transparent border border-[var(--hairline)] rounded px-1.5 py-1 text-[var(--ink)] focus:outline-none focus:border-[var(--primary)] w-full" />
            } source="text" />
            <Row label="号型" value={
              <input value={String(v.ageOrNumeric ?? op.ageOrNumeric)} onChange={(e) => upd("ageOrNumeric", e.target.value)}
                className="bg-transparent border border-[var(--hairline)] rounded px-1.5 py-1 text-[var(--ink)] focus:outline-none focus:border-[var(--primary)] w-full" />
            } source="text" />
          </div>
        </div>
